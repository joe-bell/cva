...
- `packages/cva`'s inference-sensitive types have two sharp edges (learned adding the boolean-variant shorthand): (1) a conditional type inside `CVAVariantSchema`'s mapped-type template must be non-distributive (`[T] extends [U]`) — a bare `T extends U` over the type parameter stays deferred while TS contextually types `defaultVariants`, silently widening literal values (`-1` becomes `number`) and surfacing only as far-downstream `getSchema` assignability errors; (2) never model the local-plus-composed `variants` type as a plain intersection — when both sides declare the same option key with incompatible class values (e.g. a normalized shorthand's `false: null` against a composed `false: string`) TS collapses the whole variant map to `never`, and `keyof never` is `string | number | symbol`, which poisons `VariantProps` — use the right-biased mapped merge (`MergeVariantShapes`, mirroring the runtime `mergeVariants`) instead. Diagnose both by probing intermediate types in a scratch file against the real source, not by theorizing.
