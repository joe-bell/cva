...
- `packages/cva`'s authoring types are generic over the configured concatenator via `CXInput` in `src/core.ts`, and its edges are non-obvious: `Parameters<F>[number]` picks the **last** overload of an overloaded concatenator, so one publishing a narrow form ahead of its variadic one still infers from the variadic one (pinned by a synthetic overload in `index.test.ts` — no concatenator in the real-package matrix is overloaded); the `0 extends 1 & P` check is the only reliable `any`-detection, needed so unannotated inline concatenators (contextually typed `any[]`) fall back to `ClassValue` instead of dissolving the authoring types; and non-`ClassValue`-subtype parameters (e.g. `unknown`) are clamped back to `ClassValue` so downstream `extends ClassValue` constraints stay satisfiable. Exported type pins (`CVAComponentShape`, `CVAVariantShape`) survive the generics because every new type parameter is trailing and defaulted to `ClassValue`. But any guard that accepts _components_ (`CVAComponentShape`, the `ReturnType<CVA>` checks in `Compose`/`GetSchema`) must instantiate the class-value parameter as `any` (or the config's own `T`), never leave it on the `ClassValue` default: a component authored under a narrowing concatenator (e.g. `twMerge`) has a narrower `class`/`className` prop, and props contravariance then silently rejects it from `composes`, `compose`, and `getSchema` — the `cva` preset tests never catch this, only the narrowed-concatenator cases in `concatenators.test.ts` do, so extend those whenever a new component-accepting surface is added.
- `packages/cva`'s `defaultVariants`/`compoundVariants` are typed against the _merged_ variants (local plus composed) via the trailing `Merged` parameter of `CVAComponentConfig` in `src/core.ts`, and two inference traps there are verified, not theoretical: a conditional whose check type is the merged intersection (`Merged extends CVAVariantShape ? …`) silently widens every authored default to `string` (and with it `getSchema`'s `defaultValue`), so gate on `[keyof Merged] extends [never]` instead; and mapping over `Merged` without `NoInfer` lets TypeScript reverse-infer `Variants` from the authored defaults themselves, which then accepts any value. When pinning a rejection with `@ts-expect-error`, note a `never`-typed property collapses the whole config object type, so the error can land on a sibling property rather than the offending one; write the rejected config on one line so the directive covers it wherever it lands. `packages/cva/src/concatenators.test.ts`'s composition matrix covers these; re-run it after touching the config types.
- `clsx/lite`'s published types lie: the `./lite` export reuses the full build's `clsx.d.mts` (claiming `ClassValue[]` inputs), but its runtime only appends string arguments and silently drops arrays/objects/numbers. Never rely on lite's types for safety; it works as a `cva/core` concatenator for string-authored components only because core flat-spreads its own arrays and passes authored values through verbatim (pinned in `packages/cva/src/concatenators.test.ts`).
- Both published packages (`packages/cva`, `packages/class-variance-authority`) build with [`tsdown`](https://tsdown.dev) — the full pipeline (publish transform, config feature reference, what each build-time gate checks) is documented in [CONTRIBUTING.md's Build & publish section](./CONTRIBUTING.md#build--publish-packages); read it before touching either build. The agent-critical gotchas: each `package.json`'s `exports`/`publishConfig.exports` blocks are **regenerated on every build** — never hand-edit them, change that package's `tsdown.config.mts` (or the shared `.config/tsdown.base.mts`) instead (exception: the hand-maintained `publishConfig.typesVersions` blocks — `class-variance-authority`'s `./types` and `cva`'s `./core` node10 fallbacks — which tsdown preserves but doesn't generate); a red build from the attw/publint/unused gates means the _publish shape_ broke, not the source (tsdown's pack step applies `publishConfig` — verified by breaking `dts` and watching the gates fail); and always `pnpm pack`, never `npm pack`, since only pnpm applies the `publishConfig` exports rewrite.
