...
- `packages/cva`: any type that can appear _named_ in a downstream consumer's `declaration: true` emission (e.g. `CVAComponent`/`CVAComponentShape`, reachable via a re-exported component's return type) must be `export`ed from `src/index.ts`, or consumers hit `TS4023`/`TS2459` against the shipped `dist/index.d.ts` even though the workspace source type-checks fine. `pnpm --filter cva check:declaration` (`packages/cva/test/declaration-emission/`) guards this by compiling a fixture against the real `pnpm pack` tarball — use `pnpm pack`, never `npm pack`, since only pnpm applies the package's `publishConfig` exports rewrite to `dist/`. Like `check:exports` (`attw --pack`), it validates the built `dist/`, not `src/` — the package's `check` script (`pnpm build && pnpm run '/^check:.*/'`) rebuilds first for exactly this reason, so only a direct `node test/declaration-emission/check.mjs` invocation can see a stale `dist/`.
