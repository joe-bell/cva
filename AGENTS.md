...
- `benchmark-comment.yml` (`workflow_run`) can't be exercised from a PR — GitHub always runs a `workflow_run` workflow's file as it exists on the default branch, so changes to it are invisible to the triggering PR's own run. [`test/bench/compare.ts`](./test/bench/compare.ts)'s validation/rendering logic — the part with real risk — is covered by [`test/bench/compare.test.ts`](./test/bench/compare.test.ts) instead; verify the YAML glue end-to-end with a scratch PR (ideally from a fork) after merging a change to it.
