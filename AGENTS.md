...
- The benchmark job's Actions run summary must render with `main`'s `test/bench/scripts/compare.ts` (fetched via `git show origin/main:…`), not `pnpm bench:compare` from the PR checkout — a fork can modify the PR-branch renderer and inject markdown into the summary; artifact JSON remains PR-produced either way.
