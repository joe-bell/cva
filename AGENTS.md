...
- In `CVAComponentConfig`, gate on `[keyof Merged] extends [never]`, wrap mappings in `Uninferred`, and never emit `NoInfer` because the TypeScript floor is 4.5.5. Write rejected `@ts-expect-error` configs on one line, then re-run the composition matrix in `packages/cva/src/concatenators.test.ts`; see `CVAComponentConfig`'s comment in `src/core.ts` for why.
