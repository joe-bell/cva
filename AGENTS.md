...
- **Git-worktree sessions: the pre-commit hook silently drops Prettier's fixes.** Inside a `git worktree` (where `.git` is a file, not a directory), lint-staged's "Updating Git index again" step fails with `fatal: .git/index.lock: index file open failed: Not a directory` — the commit still succeeds, but any reformatting Prettier made during the hook is **left as an unstaged modification instead of landing in the commit** (verified by committing a deliberately unformatted file). In a worktree, always run `pnpm prettier --write` on touched files and stage the result _before_ committing (which the Formatting note below requires anyway), and check `git status` right after each commit — if the hook left formatting changes behind, `git add` them and `git commit --amend` before pushing.
