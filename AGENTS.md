...
- `i-have-adhd` — shapes agent output for an ADHD reader (next action first, numbered steps, specific time estimates, state restated each turn, no preamble/recap); use when responding to any message, coding or casual. Sourced from `ayghri/i-have-adhd`; carries a local "Use when" description tweak (upstream said "Use this skill whenever"), so re-apply it and recompute the hash if `npx skills update` clobbers it. Ships an extra `agents/openai.yaml` alongside `SKILL.md` — that's the installed content, so the folder hash covers both.
