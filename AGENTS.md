...
- `benchmark-comment.yml` (`workflow_run`) can't be exercised from a PR — GitHub always runs a `workflow_run` workflow's file as it exists on the default branch, so changes to it are invisible to the triggering PR's own run. [`bench/compare.ts`](./bench/compare.ts)'s validation/rendering logic — the part with real risk — is covered by [`bench/compare.test.ts`](./bench/compare.test.ts) instead; verify the YAML glue end-to-end with a scratch PR (ideally from a fork) after merging a change to it.
- `pnpm-workspace.yaml`'s `overrides` pin `cva` and `class-variance-authority` to `workspace:*`, so installing a published npm version of either **inside** the workspace (even in a throwaway subfolder under the repo) silently resolves back to local source instead. [`bench/baselines.ts`](./bench/baselines.ts) installs benchmark baselines into a directory outside the repo entirely (e.g. `$RUNNER_TEMP`) for exactly this reason — don't "fix" a baseline install by moving it back under the repo.
