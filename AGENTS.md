...
- `pr-comment.yml` (`workflow_run`) can't be exercised from a PR — GitHub always runs a `workflow_run` workflow's file as it exists on the default branch, so changes to it are invisible to the triggering PR's own run. The risky parts — [`test/bench/compare.ts`](./test/bench/compare.ts)'s validation/rendering and [`.github/scripts/pr-comment.mjs`](./.github/scripts/pr-comment.mjs)'s section-splicing — are covered by their own test files instead; verify the YAML glue end-to-end with a scratch PR (ideally from a fork) after merging a change to it.
- Benchmark baselines must resolve npm dist-tags separately for each package: `cva` and `class-variance-authority` share a repository but have independent release lines, so a repository GitHub tag cannot identify both packages' baseline versions.
- `pnpm-workspace.yaml`'s `overrides` pin `cva` and `class-variance-authority` to `workspace:*`, so installing a published npm version of either **inside** the workspace (even in a throwaway subfolder under the repo) silently resolves back to local source instead. [`test/bench/baselines.ts`](./test/bench/baselines.ts) installs benchmark baselines into a directory outside the repo entirely (e.g. `$RUNNER_TEMP`) for exactly this reason — don't "fix" a baseline install by moving it back under the repo.
