...
- `packages/cva` builds with [`tsdown`](https://tsdown.dev) (`packages/cva/tsdown.config.mts`), which replaced the old multi-script swc + tsc pipeline. Key facts: `exports: { devExports: true }` **regenerates** `package.json`'s `exports` and `publishConfig.exports` on every build — don't hand-edit those blocks, change the config instead; the built-in attw/publint checks run at `level: "error"` and validate the _published_ manifest (tsdown's pack step applies `publishConfig`, verified by breaking `dts` and watching both checks fail the build), so a red build on "Package has no types"-style errors means the publish shape broke, not the source; the `unused` check can't see type-only dependencies, so the optional `typescript` peer is in its `ignore` list; and because root `prepare:packages` runs `pnpm build` on every `pnpm install`, these validations run on each install too — attw costs a couple of seconds there (switch to `attw: 'ci-only'` if it ever gets annoying). `size-limit` remains the bundle-size gate: tsdown's size report is informational only (per-file gzip, no budgets), while size-limit enforces brotli caps on the bundled output.
