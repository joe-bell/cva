...
- `pr-comment.yml` (`workflow_run`) can't be exercised from a PR — GitHub always runs a `workflow_run` workflow's file as it exists on the default branch, so changes to it are invisible to the triggering PR's own run. The risky parts — [`test/bench/scripts/compare.ts`](./test/bench/scripts/compare.ts)'s validation/rendering and [`.github/scripts/pr-comment.mjs`](./.github/scripts/pr-comment.mjs)'s section-splicing — are covered by their own test files instead; verify the YAML glue end-to-end with a scratch PR (ideally from a fork) after merging a change to it.
- The benchmark job's Actions run summary must render with `main`'s `test/bench/scripts/compare.ts` from a fresh `actions/checkout` of `${{ github.repository }}`/`${{ github.event.repository.default_branch }}`, not `pnpm bench:compare` or `git show origin/main:…` from the PR checkout — PR code runs first and can mutate that checkout's Git config; artifact JSON remains PR-produced either way.
