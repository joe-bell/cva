...
- `packages/cva`'s inference-sensitive types have two sharp edges (learned adding the boolean-variant shorthand): (1) a conditional type inside `CVAVariantSchema`'s mapped-type template must be non-distributive (`[T] extends [U]`) — a bare `T extends U` over the type parameter stays deferred while TS contextually types `defaultVariants`, silently widening literal values (`-1` becomes `number`) and surfacing only as far-downstream `getSchema` assignability errors; (2) never model ANY merged `variants` type as a plain intersection — local-over-composed and composed-over-composed alike — because when two sides declare the same option key with incompatible class values (a normalized shorthand's `false: null` against a composed `false: string`, or two composed shorthands' differing `true` classes) TS's unit-type intersection reduction collapses the option map to `never`, and `keyof never` is `string | number | symbol`, which poisons `VariantProps` and `getSchema`; both merge sites now use the right-biased mapped merge (`MergeVariantShapes`, and `MergedVariants` as a recursive fold over the composed tuple), mirroring the runtime `mergeVariants` reduce order. Diagnose these by probing intermediate types in a scratch file against the real source, not by theorizing — a "benign intersection" theory here was disproven by probe twice.
