...
- `packages/cva`: any type that can appear _named_ in a downstream consumer's `declaration: true` emission (e.g. `CVAComponent`/`CVAComponentShape`/`CVAVariantShape`, reachable via a re-exported component's return type or a standalone variants config) must be `export`ed from `src/index.ts`, or consumers hit `TS4023`/`TS2459`/`TS2694` against the shipped `dist/index.d.ts` even though the workspace source type-checks fine. These exports are portability-only — each carries a "you shouldn't really use it directly" JSDoc and is deliberately absent from the docs site, since we don't want to encourage direct use. `packages/cva/src/index.test.ts`'s `describe("exported types", ...)` and `describe("CVAVariantShape", ...)` blocks pin the current set by referencing each type through the `CVA.` namespace import — an unexported name fails to compile there, so `pnpm --filter cva check:tsc` catches losing one of these `export`s immediately. That only catches losing an _existing_ export, though, not a _new_ internal type leaking into a public signature; to check for that after reshaping `cva`'s public API, manually verify against the packed artifact: `pnpm --filter cva build`, then `pnpm --filter cva pack --pack-destination <tmp>` (use `pnpm pack`, never `npm pack` — only pnpm applies the package's `publishConfig` exports rewrite), extract into `<tmp>/node_modules/cva`, and compile a small consumer file re-exporting a component / `VariantProps<typeof x>` / `getSchema` result against it with `tsc --strict --declaration --emitDeclarationOnly`.
