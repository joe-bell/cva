...
- TypeScript module augmentation is program-wide, so `CVARegistry`-narrowing assertions can never live in `packages/cva`'s main test suite — one `declare module "cva/core"` in any test file rewrites the authoring types for every other test in the same compilation. Registry type-tests live in the isolated `packages/cva/type-tests/` compile instead (`.config/tsconfig.type-tests.json`, run via `check:registry`), which maps the package's own name to source via `paths` so fixtures read exactly like consumer code. Note TS 6 deprecates `baseUrl`: `paths` entries resolve relative to the tsconfig file itself.
