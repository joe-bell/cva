...
- The isolated `cva` consumer fixtures use `tsc --ignoreConfig` and import `node:` modules, so keep their explicit `--typeRoots "$package_dir/node_modules/@types" --types node` flags: TypeScript 6 otherwise excludes the installed Node declarations.
