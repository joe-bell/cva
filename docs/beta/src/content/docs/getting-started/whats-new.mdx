...
Shallow merge one or more `cva` components into a single component via the new [`composes` key](/getting-started/composing-components).

This [replaces the `compose` function](#1-compose--composes).

### 3. `getSchema`

Extract a plain-object schema (variant names, values and defaults) from a `cva` component via the new [`getSchema` function](/api-reference#getschema) — handy for generating Storybook controls/args or other UI that needs to introspect a component's variants.
...
See [Composing Components](/getting-started/composing-components) for more details and migration examples.
