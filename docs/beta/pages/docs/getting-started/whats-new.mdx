...
Merge any number of cva components can be shallow merged into a single component via the new [`compose` method](/docs/getting-started/composing-components).

### 2. `defineConfig`

Extend `cva` via the new [`defineConfig` API](/docs/api-reference#defineconfig).

Want to use `cva`/`cx` with `twMerge`? [No problem](/docs/getting-started/installation#handling-style-conflicts)!
