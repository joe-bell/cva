...
`cva` reads `options` during creation. Treat it, and everything it references, as immutable afterwards: to change a component's configuration, create a new component. A call reads only the props you pass it, plus the `cx` and `hooks` you gave [`defineConfig`](#defineconfig), which stay live. Getters with side effects, and changing the props object while the call is running, are unsupported.

A variant name starting with `_` is an [internal variant](/beta/getting-started/variants#internal-variants), with one exception: `__proto__` is rejected at compile time wherever TypeScript can still see the key. A `variants` value widened to `Record<string, Record<string, string>>` hides the key, so it accepts one.
