---
title: API Reference
description: API reference for cva, cva/config, and cva/tools.
---
...
`getSchema` and `GetSchema` remain available from `cva` and from `cva/utils`, but are deprecated in both. Import them from `cva/tools` instead.
