...
`cva/tailwindcss` helps you define component defaults that consumers can override with Tailwind CSS classes. It uses the CSS cascade instead of runtime class merging.

### `base:`

Marks a Tailwind CSS utility as a component default. Ordinary utilities take priority over `base:` utilities, while these defaults still override normal browser styles and Tailwind's `base` and `components` layers.

See the [installation guide](/beta/getting-started/installation#cvatailwindcss) for setup, usage, and caveats.
