---
title: API Reference
description: Reference for the cva, cx, and getSchema functions, and the cva/core entry point with defineConfig and CVARegistry.
---
...
By default the `cx` option and the authoring surface (`base`, variant values, `class`/`className`) are typed with `cva`'s clsx-flavored `ClassValue`. If your concatenator accepts a narrower input type, augment `CVARegistry` to adopt it project-wide: the `cx` option then accepts your concatenator as-is, and your variants are checked against what it actually supports.
...
The registered type must be a subtype of `ClassValue`, and the registration applies to the whole project (one contract per project, not per `defineConfig` call).
