...
`defineConfig` also returns a deprecated `compose` function. Use the `composes` property on `cva` instead. See [What's new?](/beta/getting-started/whats-new#1-compose--composes).
