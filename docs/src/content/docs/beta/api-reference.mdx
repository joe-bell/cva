...
Use the included `cx` instead of adding `clsx` or `classnames` for conditional class names. See the [`clsx` documentation](https://github.com/lukeed/clsx#usage) for examples of strings, arrays, and conditional objects. If you configure a different concatenator, follow its documentation for supported inputs; see [handling class conflicts](/beta/getting-started/installation#handling-class-conflicts) for the Tailwind CSS options.
