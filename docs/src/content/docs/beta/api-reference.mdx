...
A variant name starting with `_` is an [internal variant](/beta/getting-started/variants#internal-variants), with one exception: `__proto__` is rejected at compile time wherever TypeScript can still see the key, so a `variants` value widened to `Record<string, Record<string, string>>` accepts one.
