...
1. `options`
   ...
   - `composes`: shallow merge one or more other `cva` components into this one — a single component, or an array. See [Composing Components](/beta/getting-started/composing-components).
...
:::caution
**Deprecated** — use the [`composes` key](#cva) inside `cva` instead. See [Composing Components](/beta/getting-started/composing-components) for migration examples.
:::
