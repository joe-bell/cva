...
Declare an on/off variant by assigning a class value directly to the variant name. String, array, and `null` values are shorthand for a `true`/`false` map.
...
Use `null` to declare a boolean variant with no classes of its own, for example to target it from `compoundVariants`:
...
const button = cva({
  ...
  compoundVariants: [{ busy: true, disabled: true, class: "button--locked" }],
});

button({ busy: true, disabled: true });
// => "button button--disabled button--locked"
```

:::note
A plain object is always read as a variant map. To use a [`clsx` object](https://github.com/lukeed/clsx#input) as a shorthand value, wrap it in an array: `loading: [{ "cursor-wait": true }]`.
:::
