...
:::note
Although `cva` is a tiny (<BundleSize package="cva" /> compressed) library, it's best to use in an environment with server-side rendering (SSR) or static-site generation (SSG): your user probably doesn't need this JavaScript, especially for static components.
:::
...
:::tip[Prefer framework prop defaults in UI components]
For React, Svelte, or Vue components, prefer your framework's prop defaults so the same value is available to both styling and markup. For example, defaulting `disabled` in `defaultVariants` changes class selection but does not set the HTML button's `disabled` attribute. Set the prop default in the wrapper and pass that value to both the class function and the element.

Use `defaultVariants` when the class function itself should supply defaults to its callers. [`getSchema`](/beta/getting-started/tools) reads those defaults, but cannot discover defaults declared in framework wrappers. Avoid maintaining the same default in both places.
:::
