...
Put shared defaults in `defaultVariants` so every caller gets the same behavior. Omitting a prop, or passing `undefined`, uses its default. A React wrapper can forward optional variant props without repeating those defaults.
