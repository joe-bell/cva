...
:::note
`cva` is a tiny (<BundleSize package="cva" /> compressed) library. Use it with server-side rendering (SSR) or static-site generation (SSG) so static components do not send this JavaScript to users.
:::
