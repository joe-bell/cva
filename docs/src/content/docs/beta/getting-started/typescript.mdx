...
`VariantProps` contains the public variant props, not `class` or `className`. Combine it with native element props when wrapping a UI component. It omits variant names prefixed with `_`; see [Internal variants](/beta/getting-started/variants#internal-variants).
