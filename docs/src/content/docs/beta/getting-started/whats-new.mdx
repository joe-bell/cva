...
If you're already using a `cva` beta, [run the `cva-migrate` Agent Skill](/beta/getting-started/skills) to upgrade between beta versions. The rest of this page covers migrating from `class-variance-authority@0.x`.
...
Use `cva`/`cx` with `cn`, `tailwind-merge`, or your own concatenator via the [`cx` option](/beta/getting-started/installation#handling-class-conflicts).
