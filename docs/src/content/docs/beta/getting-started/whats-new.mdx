...
### 3. Root `getSchema` → `cva/tools`

`getSchema` and its `GetSchema` type are still exported from `cva`, but deprecated there. Import them from [`cva/tools`](/beta/getting-started/tools) instead:
...
An earlier beta briefly put them on `cva/utils`. That entry point still works and still re-exports the same function, but it's deprecated too: move to `cva/tools`.
