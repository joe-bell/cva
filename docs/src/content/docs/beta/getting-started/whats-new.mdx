...
Introspect components via the new [`cva/tools` entry point](/beta/getting-started/tools), which ships separately so it costs nothing unless you import it:

- `getSchema` extracts a plain-object schema (variant names, values and defaults) from a `cva` component. Use it to generate Storybook controls or any other UI that reads a component's variants.
- `pva` resolves a component's class string and its variant state, as `data-*` attributes (for example `data-intent="primary"`), from a single props object.

### 4. Internal variants

A variant name prefixed with `_` is now treated as [internal](/beta/getting-started/variants#internal-variants). It stays off a wrapping component's public props because `VariantProps`, `getSchema`, and `pva`'s `data` output omit it, while the `cva` component still accepts it.
...
### 2. Root `getSchema` → `cva/tools`

Earlier betas exported `getSchema` from the package root. That import still works, but it's deprecated: import from the [`cva/tools` entry point](/beta/getting-started/tools) instead.

```diff lang="ts"
- import { getSchema } from "cva";
