...
Calling a `cva` component is up to 500% faster than `0.7`, with no API changes. The runtime builds one list of class values per call and hands it to your concatenator once, instead of copying props and spreading arrays along the way:

- Components with variants and compound variants: 230% to 300% faster per call.
- Matching many compound variants: 360% to 500% faster.
- One component called with many different prop shapes: 350% faster.
- Components with only a `base`: 20% faster.
