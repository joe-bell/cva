...
[Run the `cva-migrate` Agent Skill](/beta/getting-started/skills) to upgrade a project, whether it's on `class-variance-authority@0.x` or an earlier `cva` beta. The rest of this page is the human-readable version of what changed since `class-variance-authority@0.x`.

## Requirements

TypeScript projects using `cva@beta` need TypeScript 6.0 or later. JavaScript usage is unaffected.

The optional `cva/tailwindcss` stylesheet needs Tailwind CSS v4.

`cva` publishes prereleases under the `beta` dist-tag and parks `latest` at `0.0.0`, so [install `cva@beta`](/beta/getting-started/installation), not `cva`.

## Features

### 1. Bring your own concatenator with `cva/config`
...
### 2. CSS defaults with `cva/tailwindcss`
...
`compose` only ever existed in the `cva` betas. If you're coming from `class-variance-authority@0.x`, skip ahead to [Enhancements](#enhancements).
...
### 8. Type imports move to the root
