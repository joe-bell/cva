...
If you are already on a `cva` beta, a coding agent can move you between beta versions: install the [`migrate` Agent Skill](/beta/getting-started/skills), then ask it to upgrade your project. Coming from `class-variance-authority@0.x` is a manual move, and the rest of this page covers it.
...
The `cva` entry is now about 1.7 kB (brotli), up from 1.4 kB. The precomputed tables are the difference.
