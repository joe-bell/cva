...
### 4. Boolean variant shorthand

Declare an on/off variant by assigning its classes directly to the variant name: `variants: { disabled: "button--disabled" }` expands to `{ disabled: { true: "button--disabled", false: null } }`. See [Boolean variants](/beta/getting-started/variants#boolean-variants).

A component's internal `config.variants` always stores the expanded form, so `getSchema` reports shorthand variants as `{ values: [true, false] }`.
