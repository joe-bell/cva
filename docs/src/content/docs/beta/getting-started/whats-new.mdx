...
Shallow merge one or more `cva` components into a single component via the new [`composes` key](/beta/getting-started/composing-components).

This [replaces the `compose` function](#1-compose--composes).

### 3. `getSchema`

Extract a plain-object schema (variant names, values and defaults) from a `cva` component via the new [`getSchema` function](/beta/api-reference#getschema) — handy for generating Storybook controls/args or other UI that needs to introspect a component's variants.

## Deprecations

### 1. `compose` → `composes`

The `compose` method is deprecated in favour of the `composes` key inside `cva`. Pass a single component directly, or pass multiple components as an array.
...
:::note
`composes` isn't always a byte-for-byte swap: when composed components declare
conflicting `defaultVariants` for the same key, `composes` applies a single
last-wins default to every component, whereas `compose` let each resolve its
own. See [Extending variants](/beta/getting-started/composing-components#extending-variants).
:::
