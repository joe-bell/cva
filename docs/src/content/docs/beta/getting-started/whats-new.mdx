...
Roll your own `cva` via the new [`defineConfig` API in `cva/core`](/beta/api-reference#cvacore).
...
### 5. Bring your own concatenator with `cva/core`

The `cva` package is a preset over the core: it wires `cva` and `cx` to `clsx`, like `class-variance-authority`. Use `cva/core` with a required `cx` option to configure `clsx/lite`, `cn`, `tailwind-merge`, or your own concatenator.
...
### 2. Preset `defineConfig` → `cva/core`

The `defineConfig` function and `DefineConfig` and `DefineConfigOptions` type imports from `cva` are deprecated but still work. Move them to `cva/core`, where `cx` is required. Add `clsx` as a direct dependency with `pnpm add clsx` to preserve the preset behavior:

```diff lang="ts"
- import { defineConfig, type DefineConfigOptions } from "cva";
+ import { defineConfig, type DefineConfigOptions } from "cva/core";
...
For `hooks.onComplete` and `hooks["cx:done"]`, move the equivalent postprocessing into `cx`. [Merging classes](/beta/getting-started/installation#merging-classes) shows `cn`, `clsx` with `tailwind-merge`, and a generic `clsx` wrapper.
