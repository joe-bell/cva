...
Extract a plain-object schema (variant names, values and defaults) from a `cva` component via the new [`getSchema` function](/beta/api-reference#getschema). Use it to generate Storybook controls or any other UI that reads a component's variants.
