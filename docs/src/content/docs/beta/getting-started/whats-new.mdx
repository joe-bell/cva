...
Use `cva`/`cx` with `tailwind-merge`, `cn`, or your own concatenator via the [`cx` option](/beta/getting-started/installation#handling-style-conflicts).
...
The `cva` package is a preset over a lean core: it wires `cva` and `cx` to `clsx`, like `class-variance-authority`. Use `cva/core` with a required `cx` option to configure `tailwind-merge`, `cn`, `clsx/lite`, or your own concatenator.
