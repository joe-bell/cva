...
Calling a `cva` component is up to 590% faster, with no API changes and no extra bytes in the bundle. The runtime builds one list of class values per call and hands it to your concatenator once, instead of copying props and spreading arrays along the way:

- Components with variants and compound variants: 240% to 300% faster per call.
- Matching many compound variants: 400% to 590% faster.
