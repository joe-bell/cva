...
- Components with variants and compound variants: 400% to 550% faster per call.
- Matching many compound variants: 700% to 1000% faster.
- One component called with many different prop shapes: 400% faster.
- Components with only a `base`: 50% faster.

The `cva` entry is now about 2 kB (brotli), up from 1.4 kB, which is what the precomputed tables cost.
