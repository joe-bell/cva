...
Configure [`defineConfig` from `cva/core`](../api-reference#cvacore) with [`tailwind-merge`](https://github.com/dcastil/tailwind-merge), a drop-in like [`cn`](https://github.com/shadcn-ui/cn), or your own function to resolve Tailwind CSS class conflicts.
...
You can write the same `clsx` + `tailwind-merge` helper yourself:
