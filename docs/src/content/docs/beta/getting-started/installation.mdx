...
Prefer to keep clsx's object syntax available? Install `clsx` alongside `tailwind-merge` and compose the two, which is what shadcn/ui's `cn` helper does:
...
An unannotated inline function like this keeps `cva`'s full `ClassValue` grammar, since nothing narrower can be inferred from it.
