...
<span id="handling-style-conflicts" />
...
For an existing `twMerge` hook, use `cx: cn` or the `clsx` and `tailwind-merge` wrapper above to preserve conditional joining and conflict resolution. For another postprocessor, keep it and join its inputs explicitly: `cx: (...inputs) => postprocess(clsx(inputs))`.
