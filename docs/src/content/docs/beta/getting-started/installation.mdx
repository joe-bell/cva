...
`cx` joins class names; it doesn't resolve conflicting CSS rules. See the [`cx` reference](/beta/api-reference#cx) for its inputs and [handling class conflicts](#handling-class-conflicts) for Tailwind CSS options.

## Tailwind CSS

Use complete class names in your variant definitions. Tailwind CSS scans source text and cannot detect interpolated names such as `bg-${tone}-500`. Map each value to a full class string instead:
