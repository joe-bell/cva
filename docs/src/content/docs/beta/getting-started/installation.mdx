...
If you want to merge Tailwind CSS classes without conflicts, swap `cva`'s concatenator via [`defineConfig` from `cva/core`](../api-reference#cvacore): pass [`tailwind-merge`](https://github.com/dcastil/tailwind-merge), a drop-in like [`cnfast`](https://github.com/aidenybai/cnfast), or your own function as the `cx` option.

Your concatenator owns the class name grammar entirely: `cva` assembles the authored values (base, matched variants, `class`/`className`) and passes them through verbatim, one argument each, so use whatever inputs your concatenator supports.

<details>

<summary>Example with tailwind-merge</summary>

`twMerge` accepts strings and arrays but not objects, so it's narrower than `cva`'s default clsx-flavored contract. Augment `CVARegistry` to adopt tailwind-merge's own input type: `cx: twMerge` then typechecks as-is, and your variants are checked against what `twMerge` actually supports.
...
Prefer to keep clsx's object syntax available in your variants? Skip the augmentation and compose the two instead: `cx: (...inputs) => twMerge(clsx(inputs))`.
