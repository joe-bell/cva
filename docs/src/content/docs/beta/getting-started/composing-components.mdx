...
[Boolean shorthand](/beta/getting-started/variants#boolean-variants) variants compose exactly like their longhand expansion. Two components declaring the same shorthand variant extend one another:

```ts
const toggle = cva({
  base: "toggle",
  variants: { disabled: "toggle--disabled" },
});
const button = cva({
  base: "button",
  variants: { disabled: "button--disabled" },
});

const combined = cva({ composes: [toggle, button] });

combined({ disabled: true });
// => "toggle toggle--disabled button button--disabled"
```
