...
Shallow merge one or more `cva` components into a single component with the `composes` key. Pass a single component directly, or pass multiple components as an array:
...
`defaultVariants` follow a last-wins merge. If multiple composed components declare a default for the same variant, the last one in the array wins, and a local `defaultVariants` on the composing component wins over all of them. `cva` applies that value to every composed component, not just the one that declared it:
