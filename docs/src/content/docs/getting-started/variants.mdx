...
:::note
Although `cva` is a tiny (<BundleSize package="class-variance-authority" /> compressed) library, it's best to use in an environment with server-side rendering (SSR) or static-site generation (SSG): your user probably doesn't need this JavaScript, especially for static components.
:::
