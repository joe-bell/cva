...
Results are posted as a sticky PR comment (updated in place on every push, including from forks) by a separate `pr-comment` workflow — see that file's header comment for why it's split out, how the untrusted artifact it reads is validated, and how to add a section of your own to the same comment from a future CI step. The benchmark job also writes the same table to the Actions run summary, but pins the renderer to `main`'s `compare.ts` (artifact JSON is still PR-produced). This is **informational only**: a regression doesn't fail CI, so treat it as a signal to investigate, not a gate.
