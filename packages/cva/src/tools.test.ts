...
describe("getSchema", () => {
  ...
  test("should reject an explicitly empty variants object", () => {
    // `variants: {}` leaves `[keyof Merged] extends [never]` true, so
    // `CVAComponentConfig` resolves `config` to `never` and the call is
    // rejected. Base behaves the same way; this pins it against a `GetSchema`
    // rewrite that would quietly start accepting it.
    const component = cva({ variants: {} });

    // @ts-expect-error: an empty variants object is not introspectable
