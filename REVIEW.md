...
- **A newly-nameable internal type must be exported.** Per [`CONTRIBUTING.md`](./CONTRIBUTING.md#style-guide): a type that appears _named_ in a consumer's generated `.d.ts` under `declaration: true` breaks their build with a `TS4023`/`TS2459`-family error if it isn't exported from the public entry point that exposes the signature (`cva`, `cva/config`, or `cva/tools`) — while `cva`'s own build stays green. The `describe("exported types", …)` and `describe("CVAVariantShape", …)` blocks in `packages/cva/src/index.test.ts` pin the _existing_ portability exports (`CVAComponent`, `CVAComponentShape`, `CVAVariantShape`), so losing one fails `pnpm --filter cva check:tsc`. `pnpm --filter cva test:consumer` checks the current public ESM/CJS fixtures and emitted declarations in a layout that does not hoist `clsx`, so a preset or signature that infers a dependency type fails with TS2883; it also verifies `cva/utils` is rejected by the package export map. A new public signature beyond those fixtures still needs review for newly-nameable types.
...
- **Inference can widen silently and still compile.** `pnpm check` only proves `tsc` is happy. Whether `VariantProps<typeof x>` still resolves to a literal union rather than `string`, or `getSchema` still names every variant, is pinned only by the `expectTypeOf(…).toEqualTypeOf<…>()` assertions in the test files. Treat a loosened assertion (`toEqualTypeOf` → `toExtend`), a deleted one, or a deleted `@ts-expect-error` as part of the diff to review, not as test housekeeping.
- **Authoring-type changes need a measurement and a packed consumer.** Same accept/reject behaviour and consumer declarations, no instantiation regression on plain, composed, `getSchema` and direct-interface fixtures, and a packed consumer proving nothing new needs exporting.
...
- Local `defaultVariants` beat composed ones. Type side: the `Omit<MergedDefaultVariants<…>, keyof CVADefaultVariants<Config>> & CVADefaultVariants<Config>` in the `CVA` interface. Runtime side: the authored config is the last source folded into `defaults` by `cva`'s definition-time merge loop. Same rule, two places.
...
- **Published `typesVersions` fallbacks** — `class-variance-authority`'s `./types` and `cva`'s `./config` and `./tools` node10 fallbacks — are hand-maintained. tsdown preserves them but doesn't generate them, so a deletion never comes back, and a new subpath entry never appears on its own.
...
- `bundlesize` enforces the numbers, not the choice of numbers. The `size-limit` blocks cap the `cva` preset at `1.65KB`, `cva/config` at `1.45KB`, `cva/tools` at `0.35KB`, and `class-variance-authority` at `1.2KB`. A PR that raises its own limit to fit gets a green job — that is a review decision, weighed against the "performance & minimal footprint" project goal.
